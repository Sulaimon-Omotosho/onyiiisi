...
export default function RelatedProducts({
  ...
  const prevSaleMob = () => {
    ...
    const newIndex = isFirstSale ? relatedProducts.length - 1 : currentIndex - 1
    setCurrentIndex(newIndex)
  }

  const nextSaleMob = () => {
    const isLastSale = currentIndex === relatedProducts.length - 1
  ...
  const prevSale = () => {
    const isFirstSale = currentIndex === 0
    const newIndex = isFirstSale ? relatedProducts.length - 3 : currentIndex - 3
    setCurrentIndex(newIndex)
  }

  const nextSale = () => {
    const isLastSale = currentIndex === relatedProducts.length - 3
    const newIndex = isLastSale ? 0 : currentIndex + 3
    setCurrentIndex(newIndex)
  }
  ...
  return (
    <div className='bg-[rgb(56,22,10)] my-10 py-5 '>
      ...
      <div className='flex flex-col md:flex-row justify-around content-center items-center relative  '>
        ...
        {/* <ChevronLeft
          onClick={prevSale}
          className='absolute left-0 cursor-pointer hover:scale-150 ease-in-out duration-300 z-50 w-10 h-10'
        />
        <ChevronRight
          onClick={nextSale}
          className='absolute right-0 cursor-pointer hover:scale-150 ease-in-out duration-300 z-50 w-10 h-10'
        /> */}
        ...
        <ChevronLeft
          onClick={prevSaleMob}
          className='absolute left-3 cursor-pointer hover:scale-150 ease-in-out text-white duration-300 z-50 md:hidden'
        />
        <ChevronRight
          onClick={nextSaleMob}
          className='absolute right-3 text-white cursor-pointer hover:scale-150 ease-in-out duration-300 z-50 md:hidden'
        />
      </div>
    </div>
  )
